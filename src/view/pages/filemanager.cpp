...
//  qDebug() << "released key: " << event->key();
  ...
  switch (e->key()) {
    ...
    case Qt::Key_Insert:
         if (dirs->hasFocus()) createDirectory();
         else if (files->hasFocus()) {
            //TODO: create new file and open gcode editor
            qDebug() << "TODO: TableView ?!?";
            } break;
    case Qt::Key_Delete:
         if (dirs->hasFocus()) {
    ...
    case Qt::Key_F5:
         if (files->hasFocus()) {   // start copy file
    ...
    case Qt::Key_F6: {
         if (files->hasFocus()) {   // start move file
            QModelIndexList  mil    = files->selectionModel()->selectedRows();
            QModelIndex      cur    = mil.size() ? mil.at(0) : QModelIndex();
            QModelIndex      srcIdx = pxFiles->mapToSource(cur);
            const QFileInfo& fi     = fileModel->fileInfo(srcIdx.row());
         ...
         else if (dirs->hasFocus()) renameDirectory();
         } break;
    default: break;
    }
  }
